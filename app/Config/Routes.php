...
$routes->group('backend', function ($routes) {
    ...
    $routes->group('data-verifikator', function ($routes) {
        $root_groups = "Backend\Pendataan\DataVerifikator";
        //pages
        $routes->add('/', $root_groups . '::index', ['filter' => 'auth']);
        $routes->add('add', $root_groups . '::add', ['filter' => 'auth']);
        $routes->get('edit', $root_groups . '::edit', ['filter' => 'auth']);
        //process
        $routes->post('insert', $root_groups . '::insert', ['filter' => 'auth']);
        $routes->post('preview', $root_groups . '::preview', ['filter' => 'auth']);
        $routes->get('delete', $root_groups . '::delete', ['filter' => 'auth']);
        $routes->get('export', $root_groups . '::export', ['filter' => 'auth']);
    });
...
$routes->group('inbox-pertanyaan', function ($routes) {
    $root_groups = "Backend\InboxPertanyaan";
    //pages
    $routes->add('/', $root_groups . '::index', ['filter' => 'auth']);
});

$routes->group('faq', function ($routes) {
    $root_folder = "Frontend\Faq\Main";
    //pages
    $routes->add('/', $root_folder . '::index');
    $routes->post('ajukan-pertanyaan', $root_folder . '::ask');
});


$routes->group('rekon', function ($routes) {
    $root_folder = "Frontend\Rekon\Main";
    //pages
    $routes->add('/', $root_folder . '::index');
    $routes->post('ajukan', $root_folder . '::ajukan');
    $routes->post('cari', $root_folder . '::cari');
});

$routes->group('formasi', function ($routes) {
    $root_folder = "Frontend\Formasi\Main";
    //pages
    $routes->get('(:segment)/(:segment)', $root_folder . '::index/$1/$2');
});

$routes->group('statistik', function ($routes) {
    $root_folder = "Frontend\Statistik\Main";
    //pages
    $routes->add('/', $root_folder . '::index');
});
